    ...
    <div>
      ...
      <section className="pt-24 pb-20 px-6 text-center max-sm:pt-16 max-sm:pb-14">
        <div className="max-w-[720px] mx-auto">
          <h1 className="text-[52px] font-extrabold leading-[1.25] tracking-[-0.04em] text-foreground mb-5 max-sm:text-[34px]">
            대학생을 위한<br />
            <span className="text-primary">자료 플랫폼</span>
          </h1>
          ...
          <form onSubmit={handleSearch} className="max-w-[580px] mx-auto">
            <div className="relative">
              ...
              <button
                type="submit"
                className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-xl px-6 h-11 bg-primary !text-white font-semibold text-sm hover:bg-primary-dark transition-colors max-sm:px-4"
              >
                검색
              </button>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 max-lg:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1">
            {universities.filter((univ) => univ.id === "korea").map((univ) => (
              <Link
                to={univ.path}
                key={univ.id}
                className="flex items-center gap-4 p-5 rounded-2xl border border-border bg-white transition-all hover:-translate-y-0.5 hover:shadow-sm"
              >
                <img
                  src={univ.logo}
                  alt={univ.name}
      ...
      <section className="py-24 max-sm:py-16">
        <div className="max-w-[560px] mx-auto text-center px-6">
          ...
          <Link
            to="/upload"
            className="inline-flex items-center justify-center h-14 px-10 rounded-xl bg-primary !text-white font-bold text-base hover:bg-primary-dark transition-colors"
          >
            지금 판매하기
          </Link>
        </div>
