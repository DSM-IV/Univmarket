...
export default function YonseiPage() {
        ...
        <div className="max-w-[640px] mx-auto">
          <h1 className="text-[52px] font-extrabold leading-[1.25] text-foreground mb-3 max-sm:text-[34px]">
            <span className="inline-flex items-baseline">
              <span className="inline-block transition-all duration-700 ease-in-out" style={{ color: "#2E8BC0", transform: transformed ? "translateX(0)" : "translateX(3.5em)" }}>File</span>
              ...
              <span className="inline-block transition-all duration-700 ease-in-out" style={{ width: transformed ? "0.3em" : "0" }} />
              {"Yonsei".split("").map((ch, i) => (
                <span key={i} className="inline-block transition-all duration-700 ease-in-out overflow-hidden" style={{ maxWidth: transformed ? "0.65em" : (ch === "Y" ? "0.65em" : "0"), opacity: transformed ? 1 : (ch === "Y" ? 1 : 0), color: "#003876" }}>
                  {ch}
                </span>
              ))}
            </span>
          </h1>
          <p className="text-[22px] font-bold text-foreground mb-2 max-sm:text-lg transition-all duration-500" style={{ opacity: transformed ? 1 : 0, transform: transformed ? "translateY(0)" : "translateY(8px)" }}>
            더이상 자료 찾아서 헤메지 마세요.
          </p>
          <p className="text-lg text-muted-foreground mb-10 max-sm:text-[15px] max-sm:mb-8">연세대 학생들이 만든 양질의 공부자료를 만나보세요.</p>
      ...
      <section className="py-16 max-sm:py-12">
        <div className="max-w-[1140px] mx-auto px-6">
          ...
          <div className="grid grid-cols-4 gap-3 max-lg:grid-cols-2">
            {categories.map((cat) => (
              <Link to={`/browse?category=${encodeURIComponent(cat.name)}`} key={cat.name} className="flex flex-col items-center gap-2.5 py-7 px-4 bg-muted rounded-xl transition-all hover:bg-primary/5 hover:-translate-y-0.5">
                <span className="text-[32px]">{cat.icon}</span>
                <span className="text-[15px] font-semibold">{cat.name}</span>
                <span className="text-xs text-muted-foreground text-center leading-snug">{cat.examples}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <section className="py-16 bg-muted max-sm:py-12">
          <div className="max-w-[1140px] mx-auto px-6">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link to="/browse?sort=popular" className="flex items-center gap-0.5 text-sm font-semibold text-muted-foreground hover:text-primary transition-colors shrink-0">전체보기<ChevronRight className="w-4 h-4" /></Link>
            </div>
            <div className="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-3">
              {popularMaterials.map((m) => (<MaterialCard key={m.id} material={m} rating={reviewStats[m.id]?.avgRating} reviewCount={reviewStats[m.id]?.reviewCount} />))}
            </div>
          </div>
        </section>
      )}
        ...
        <section className="py-16 max-sm:py-12">
          <div className="max-w-[1140px] mx-auto px-6">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link to="/browse?sort=recent" className="flex items-center gap-0.5 text-sm font-semibold text-muted-foreground hover:text-primary transition-colors shrink-0">전체보기<ChevronRight className="w-4 h-4" /></Link>
            </div>
            <div className="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1 max-sm:gap-3">
              {recentMaterials.map((m) => (<MaterialCard key={m.id} material={m} rating={reviewStats[m.id]?.avgRating} reviewCount={reviewStats[m.id]?.reviewCount} />))}
            </div>
          </div>
        </section>
      )}
