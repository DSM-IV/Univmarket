...
export default function SkkuPage() {
        ...
        <div className="max-w-[640px] mx-auto">
          <h1 className="text-[52px] font-extrabold leading-[1.25] text-foreground mb-3 max-sm:text-[34px]">
            <span className="inline-flex items-baseline">
              ...
              <span className="inline-block transition-all duration-700 ease-in-out" style={{ width: transformed ? "0.3em" : "0" }} />
              {"SKK".split("").map((ch, i) => (
                <span key={i} className="inline-block transition-all duration-700 ease-in-out overflow-hidden" style={{ maxWidth: transformed ? "0.65em" : "0", opacity: transformed ? 1 : 0, color: "#004B49" }}>{ch}</span>
              ))}
              <span className="inline-block transition-all duration-700 ease-in-out" style={{ color: transformed ? "#004B49" : "#1B3A5C", transform: transformed ? "translateX(0)" : "translateX(-2.5em)" }}>U</span>
            </span>
          </h1>
          ...
          <p className="text-lg text-muted-foreground mb-10 max-sm:text-[15px] max-sm:mb-8">성균관대 학생들이 만든 양질의 공부자료를 만나보세요.</p>
